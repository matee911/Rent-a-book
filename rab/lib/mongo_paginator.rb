# fork of http://github.com/lholden/merb-pagination
# added configuration in yaml
# changed paginate to accept mongomapper paginated collection
require 'builder'
module Merb
  module PaginationHelper
    ...
    PAGINATOR_CONFIG = YAML.load_file(File.join(Merb.root, 'config', 'paginator.yml'))
    ...
    def paginate(collection, opts = {})
      options = {
        ...
        :page_param      => 'page',
        :url             => '',
        :show_empty_prev => true,
        :show_empty_next => true,
        :li_class        => '',
        :link_class      => '',
        :current_class   => 'current_page'
      }.
      merge(PAGINATOR_CONFIG).
      merge(opts)
      ...
      pages = { 
        :all => (1 .. collection.total_pages).to_a, 
        :left => [], 
        :center => [], 
        :right => [] 
      }
      ...
      Builder::XmlMarkup.new.div(:class => options[:class]) do |b|
        ...
        b << (current_page <= 1 ? (options[:show_empty_prev] ? options[:prev_label] : '') : %Q{<a href="#{url}=#{current_page -1}">#{options[:prev_label]}</a>})
        ...
        b.ul do
          [pages[:left], pages[:center], pages[:right]].each do |p|
            p.each do |page_number|
              ...
              when current_page
                b.li(page_number, :class=>options[:current_class])
              else
                b.li(:class=>options[:li_class]) { b.a(page_number, :href=>"#{url}=#{page_number}", :class=>options[:link_class]) }
              end
            end
          end
        end
        
        b << (current_page >= page_count ? (options[:show_empty_next] ? options[:next_label] : '') : %Q{<a href="#{url}=#{current_page +1}">#{options[:next_label]}</a>})
      end
    end
    private
